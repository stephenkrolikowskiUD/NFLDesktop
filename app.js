...
function renderDataWarnings(){
  const warnings=visibleDataWarnings();
...
function scheduleRowStartMs(row){
  const fullStart=rowField(row,"commence_time","start_time");
  const parsedFull=parseStartMs(fullStart);
  if(Number.isFinite(parsedFull))return parsedFull;
  const gameDate=String(rowField(row,"game_date","GAME_DATE","gameday","GAME_DAY","date","DATE")||"").trim();
  const gameTime=String(rowField(row,"game_time","GAME_TIME")||"").trim();
  if(gameDate&&gameTime){
    const combined=parseEasternTimestamp(`${gameDate} ${gameTime}`);
    if(Number.isFinite(combined))return combined;
  }
  const parsedTimeOnly=parseStartMs(gameTime);
  return Number.isFinite(parsedTimeOnly)?parsedTimeOnly:null;
}
...
function tierClassForConfidence(conf){
  const key=normalizeConfidence(conf);
  return key==="SMASH"?"smash":key==="STRONG"?"strong":"lean";
}
function oddsMetaText(odds){
  return odds!==""&&odds!==null&&odds!==undefined?fmtOdds(odds):"price pending";
}
...
function renderTonightShortlist(){
  ...
  return`<section class="shortlist-shell"><div class="shortlist-head"><div><div class="analysis-eyebrow">This week's decision board</div><div class="shortlist-title">This Week's Shortlist</div><div class="shortlist-sub">Qualified opportunities only: playable price, real sample, no lineup risk, and no active disagreement with the board's own gates.</div></div><div class="shortlist-rule">Ranked by conviction</div></div><div class="shortlist-summary-row"><span><strong>${rows.length}</strong> qualified</span><span><strong>+${(avgEdge*100).toFixed(1)}%</strong> average edge</span><span><strong>${aiBacked}</strong> AI-backed</span><span><strong>${markets}</strong> markets</span></div>${renderShortlistTray()}<div class="shortlist-grid">${rows.map((row,index)=>{
...
function renderGameMarketsBoard(team,rows,options={}){
  ...
  const cards=gameEntries.map(({game,rows:gameRows,first,bestPick})=>{
    ...
    const marketRowsHTML=gameRows
      .slice()
      .sort((a,b)=>{
        const aType=String(rowField(a,"MARKET_TYPE")).trim().toUpperCase();
        const bType=String(rowField(b,"MARKET_TYPE")).trim().toUpperCase();
        ...
        if((order[aType]??9)!==(order[bType]??9))return (order[aType]??9)-(order[bType]??9);
        return gameMarketSelectionText(a).localeCompare(gameMarketSelectionText(b));
      })
      .map(row=>{
        const type=String(rowField(row,"MARKET_TYPE")).trim().toUpperCase();
        ...
        const confidence=matchingPick?normalizeConfidence(rowField(matchingPick,"confidence")):"";
        const tierClass=confidence==="SMASH"?"smash":confidence==="STRONG"?"strong":confidence==="LEAN"?"lean":"";
        const edge=matchingPick?teamMarketEdgeLabel(matchingPick):"";
        const modelHit=matchingPick?Number(rowField(matchingPick,"MODEL_HIT_RATE")):NaN;
        const implied=matchingPick?Number(rowField(matchingPick,"IMPLIED_PROBABILITY")):NaN;
        const fairOdds=matchingPick?rowField(matchingPick,"FAIR_ODDS","FAIR_LINE"):"";
        const rationale=matchingPick?String(rowField(matchingPick,"rationale")||"").trim():"";
        const bookLabel=rowIsBaseline?"Opening board":rowBook;
        const oddsText=rowIsBaseline
          ?type==="MONEYLINE"?"Baseline":"Baseline line"
          :gameMarketOddsLabel(odds);
        const edgeText=edge||"No ranked lean";
        const modelText=Number.isFinite(modelHit)?`${Math.round(modelHit*100)}% model`:"Model pending";
        const marketText=Number.isFinite(implied)?`${Math.round(implied*100)}% market`:"Market pending";
        const fairText=fairOdds!==""&&fairOdds!=null?`fair ${fmtOdds(fairOdds)}`:"";
        const detailText=rationale||teamMarketSummary(matchingPick)||"Opening preseason market context.";
        const teamMeta=type==="TOTAL"?"Game total":String(rowField(row,"TEAM")||"").trim().toUpperCase()||"Team side";
        return `<details class="game-market-matrix-row${isPreferred?" preferred":""}${tierClass?` ${tierClass}`:""}">
          <summary class="game-market-matrix-summary">
            <span class="game-market-cell game-market-cell-type">${esc(gameMarketTypeShortLabel(type))}</span>
            <span class="game-market-cell game-market-cell-selection">${esc(selection)}</span>
            <span class="game-market-cell game-market-cell-team">${esc(teamMeta)}</span>
            <span class="game-market-cell game-market-cell-price">${esc(oddsText)}</span>
            <span class="game-market-cell game-market-cell-edge">${esc(edgeText)}</span>
            <span class="game-market-cell game-market-cell-book">${esc(bookLabel)}</span>
            <span class="game-market-cell game-market-cell-status">${isPreferred?`<span class="market-status-dot ${tierClass||"preferred"}"></span>`:"<span class=\"market-status-dot idle\"></span>"}</span>
          </summary>
          <div class="game-market-matrix-detail">
            <div class="game-market-matrix-detail-grid">
              <div>${esc(detailText)}</div>
              <div class="game-market-detail-metrics">${esc([modelText,marketText,fairText].filter(Boolean).join(" · "))}</div>
            </div>
            ${featuredSource&&matchingPick?`<div class="game-market-detail-source">${esc(featuredSource)}${confidence?` · ${esc(confidence)}`:""}</div>`:""}
          </div>
        </details>`;
      }).join("");
    const featuredSummary=bestPick
      ?`<div class="game-market-hero"><div class="game-market-hero-main"><div class="game-market-hero-label">Top play</div><div class="game-market-hero-selection">${esc(featuredSelection||game)}</div><div class="game-market-hero-reason">${esc(featuredReason||"Current preseason board lean.")} ${esc(teamMarketSummary(bestPick))}.${featuredSource?` ${featuredSource}.`:""}</div></div><div class="game-market-hero-side"><div class="game-market-hero-tier">${esc(featuredConfidence||"LEAN")}</div><div class="game-market-hero-rank">#${esc(String(rowField(bestPick,"rank")||"—"))}</div></div></div>`
      :"";
    return `<section class="game-market-shell">
      <div class="game-market-shell-head">
        ...
        <div class="game-market-shell-tape">
          <span>${esc(away||"AWAY")} @ ${esc(home||"HOME")}</span>
          <span>${esc(isBaselineBook?"Opening board":book)}</span>
          <span>${gameRows.length} rows</span>
        </div>
      </div>
      ${featuredSummary}
      <div class="game-market-matrix">
        <div class="game-market-matrix-head">
          <span>Market</span>
          <span>Selection</span>
          <span>Side</span>
          <span>Price</span>
...
function getDraftSlateGames(){
  return getMemo("draftSlateGames",()=>{
    ...
    for(const row of st.schedule||[]){
      ...
      const startMs=scheduleRowStartMs(row);
...
function renderDraftSlateSelector(){
  ...
  const summary=selectedGames.length===games.length&&games.length
    ?`All ${games.length} upcoming games · ${range}`
...
function pickProvenanceHTML(model){
  ...
  return `<div class="pick-provenance" title="${esc(source.description)}"><span class="pick-provenance-label ${source.className}">${esc(source.label)}</span>${source.consensusCount>=2?`<span class="pick-provenance-sep">·</span><span class="pick-provenance-note">run consensus ${source.consensusCount}</span>`:""}</div>`;
}
...
function pickFooterLine(model){
  const p=model.pk;
  if(model.isGameMarket){
    return `${esc(rowField(p,"PICK_BOOK")||"opening board")} · ${esc(propTypeLabel(p.prop_type))} · ${esc(selectionMethodLabel(rowField(p,"SELECTION_METHOD")||"VALIDATED_MODEL"))}`;
  }
  return `${esc(propTypeLabel(p.prop_type))} · ${esc(model.provenance.label)}${model.provenance.consensusCount>=2?` · consensus ${model.provenance.consensusCount}`:""}`;
}
...
function renderFeaturedPick(model){
  ...
  const callText=model.isGameMarket
    ?esc(oddsMetaText(rowField(p,"PICK_ODDS")))
...
function renderModelPicksView(convergenceHTML,{title=weeklyPickLabel(),subtitle="Ranked best bets across every game this week with a live market.",rows=st.weeklyPicks,sourceNote=""}={}){
  ...
  if(hits+misses){
    html+=`<div class="model-picks-record"><span class="model-picks-record-label">Current record</span><strong class="model-picks-record-value ${hits>=misses?"positive":"neutral"}">${hits}-${misses}</strong></div>`;
  }
...
function renderPropExplorerView(){
  let html="";
      ...
      html=`<div style="padding:14px 16px 0"><div class="analysis-eyebrow">Player prop research</div><div style="font-family:'Barlow Condensed',system-ui,sans-serif;font-size:var(--t-xl);font-weight:800;color:var(--ink-0)">Market Explorer</div><div style="color:var(--ink-muted);font-size:var(--t-xs);margin-top:2px">Use this page for player props once books open them. Team-side spreads, moneylines, and totals now live in their own Game Markets tab.</div></div><div class="props-toolbar">
        <div class="props-control props-control-search"><label for="propsSearchInput">Player or team</label><input type="text" id="propsSearchInput" placeholder="Search Mahomes, KC, Chiefs..." value="${esc(st.propsSearch)}"/></div>
...
function renderMethodPage(activeTab){
  return `<div id="pg-method" class="page ${activeTab==="method"?"active":""}">
    <div style="padding:16px"><div style="text-align:center;margin-bottom:16px"><div style="font-size:28px;margin-bottom:4px">🏈</div><div style="color:var(--accent);font-weight:800;font-size:var(--t-md)">How This Dashboard Works</div><div style="color:var(--accent-soft);font-size:var(--t-xs);margin-top:4px">Under the hood of the NFL DFS engine</div></div>
      ...
      <div class="card" style="margin-bottom:10px;border:1px solid var(--border-1)"><div class="card-title">🙏 Credits</div><div style="font-size:var(--t-sm);color:var(--ink-1);line-height:1.6"><div>Built by <span style="color:var(--accent);font-weight:700">Stephen Krolikowski</span></div><div>AI review / grading layer (still planned for NFL): <span style="color:var(--accent);font-weight:700">Gemini 3.6 Flash</span> · App: <span style="color:var(--accent);font-weight:700">Claude</span> by Anthropic</div><div>Engine + dashboard fixes with <span style="color:var(--accent);font-weight:700">Codex</span> by OpenAI</div><div>Props: <span style="color:var(--accent);font-weight:700">The Odds API</span> · Data: <span style="color:var(--accent);font-weight:700">nflverse</span></div></div></div>
