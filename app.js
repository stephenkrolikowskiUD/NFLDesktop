...
function renderCLVSummary(rows,w){
  ...
  if(!state.meaningful){
    ...
    return `<div style="padding:0 16px 10px"><div class="card" style="border-left:3px solid var(--warn-line)"><div style="color:var(--accent);font-size:var(--t-sm);font-weight:800;margin-bottom:4px">CLV Not Live Yet</div><div style="font-size:var(--t-xs);color:var(--ink-1);line-height:1.6">NFL CLV is hidden until the workflow captures enough real opening-versus-current price movement to be meaningful. ${esc(reason)} Right now this repo has price-discipline checks, but not a trustworthy end-to-end CLV capture loop.</div></div></div>`;
  }
